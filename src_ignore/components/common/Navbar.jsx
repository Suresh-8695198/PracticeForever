                    ...
                    <>
                        ...
                        >
                            ...
                            <nav className='flex-1 overflow-y-auto px-2 pb-4'>
                                <Link to='/' onClick={() => setMobileOpen(false)} className={`flex items-center h-11 px-3 text-[14px] font-medium rounded-lg mb-0.5 ${isDark ? 'text-gray-200 hover:bg-[#1a1a1a]' : 'text-gray-800 hover:bg-gray-50'}`}>
                                    Home
                                </Link>
                                    ...
                                    return (
                                        <div key={cat.title} className='mb-0.5'>
                                            {cat.subcategories ? (
                                                ...
                                                <Link to={cat.path} onClick={() => setMobileOpen(false)} className={`flex items-center gap-2.5 h-11 px-3 text-[14px] font-medium rounded-lg ${isDark ? 'text-gray-200 hover:bg-[#1a1a1a]' : 'text-gray-800 hover:bg-gray-50'}`}>
                                                    <div className={`w-6 h-6 rounded-md flex items-center justify-center ${cat.bgCls} ${cat.colorCls}`}>
                                                        <CatIcon size={13} />
                                                    </div>
                                                    {cat.title}
                            ...
                            {!isAuthenticated && (
                                <div className={`p-3 border-t flex flex-col gap-2 ${isDark ? 'border-[#222]' : 'border-gray-100'}`}>
                                    <Link to='/register' onClick={() => setMobileOpen(false)} className='flex items-center justify-center h-10 rounded-lg bg-[#FFC107] text-black font-semibold text-[14px]'>
                                        Register Free
                                    </Link>
                                    <Link to='/login' onClick={() => setMobileOpen(false)} className={`flex items-center justify-center h-10 rounded-lg border text-[14px] font-medium ${isDark ? 'border-[#333] text-gray-200' : 'border-gray-200 text-gray-700'}`}>
                                        Log In
                                    </Link>
                                </div>
                            )}
