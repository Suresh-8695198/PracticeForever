...
const Home = () => {
  ...
  const SectionHead = ({ title, sub, to, linkLabel, iconUrl }) => (
    <div className={`flex flex-col sm:flex-row sm:items-end justify-between gap-4 pb-6 mb-10 border-b ${isDark ? 'border-white/5' : 'border-gray-100'}`}>
      ...
      {to && (
        <Link to={to} className="flex items-center gap-1.5 text-[13px] font-bold whitespace-nowrap group pb-1">
  ...
  return (
    <div className={`min-h-screen pt-[104px] ${bg}`} style={bodyFont}>
          ...
          <div className="flex items-center gap-8 lg:gap-12">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-3">
                <Link to="/exams" className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#FFC107] text-black text-[13px] font-bold rounded hover:bg-amber-400 transition-colors" style={bodyFont}>
                  Start Preparation
                </Link>
                <Link to="/mock-tests" className={`inline-flex items-center gap-2 px-5 py-2.5 text-[13px] font-semibold rounded border transition-colors ${isDark ? 'border-[#333] text-gray-200 hover:border-[#555]' : 'border-gray-300 text-gray-700 hover:border-gray-400 hover:bg-gray-50'}`} style={bodyFont}>
                  Take Mock Test
                </Link>
                <Link to="/current-affairs/daily" className={`inline-flex items-center gap-2 px-5 py-2.5 text-[13px] font-semibold rounded border transition-colors ${isDark ? 'border-[#333] text-gray-200 hover:border-[#555]' : 'border-gray-300 text-gray-700 hover:border-gray-400 hover:bg-gray-50'}`} style={bodyFont}>
      ...
      {/* ══ STATS BAR ══ */}
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #FFC107 0%, #F59E0B 60%, #D97706 100%)' }}>
        {/* SVG grid squares pattern */}
        <svg className="absolute inset-0 w-full h-full pointer-events-none" aria-hidden="true">
          <defs>
            <pattern id="stats-grid" x="0" y="0" width="28" height="28" patternUnits="userSpaceOnUse">
              <rect x="0" y="0" width="26" height="26" rx="4" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="1" />
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#stats-grid)" />
        </svg>
        {/* Glow accents */}
        <div className="absolute -left-16 -top-16 w-56 h-56 rounded-full pointer-events-none" style={{ background: 'rgba(255,255,255,0.12)', filter: 'blur(40px)' }} />
        <div className="absolute -right-16 -bottom-16 w-56 h-56 rounded-full pointer-events-none" style={{ background: 'rgba(0,0,0,0.08)', filter: 'blur(40px)' }} />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 sm:grid-cols-4">
            {[
              { num: '1M+', lbl: 'Active Students' },
              { num: '8,100+', lbl: 'Practice Questions' },
              { num: '5M+', lbl: 'Mock Tests Taken' },
              { num: '40%', lbl: 'Avg Score Improvement' },
            ].map(({ num, lbl }, i) => (
              <div key={lbl} className="px-6 py-5 text-center" style={{ borderLeft: i !== 0 ? '1px solid rgba(255,255,255,0.25)' : 'none' }}>
                <em style={{ fontStyle: 'normal', display: 'block', fontSize: 22, fontWeight: 800, color: '#fff', ...headFont, letterSpacing: '-0.02em' }}>{num}</em>
                <em style={{ fontStyle: 'normal', display: 'block', fontSize: 11.5, fontWeight: 500, color: 'rgba(255,255,255,0.85)', marginTop: 3, ...bodyFont }}>{lbl}</em>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row gap-10">
            <div className="flex-1">
              ...
              <Link to="/aptitude" className="mt-6 inline-flex items-center gap-2 px-6 py-3 bg-[#FFC107] text-black text-[13.5px] font-bold rounded-lg hover:bg-amber-400 transition-all duration-200">
            ...
            <div className={`lg:w-[300px] shrink-0 rounded-xl border p-7 transition-all duration-400 ${isDark ? 'bg-[#141414] border-[#2a2a2a]' : 'bg-white border-gray-100'}`}>
              ...
              <Link to="/mock-tests" className={`mt-8 block w-full text-center py-2.5 rounded-lg text-[13px] font-bold transition-colors ${isDark ? 'bg-white/10 text-amber-400 hover:bg-white/20' : 'bg-[#ec4899] text-white hover:bg-[#db2777]'}`}>
                View Leaderboard
              </Link>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start mt-2">
            ...
            <div className={`rounded-xl p-8 border transition-all duration-400 ${isDark ? 'bg-[#141414] border-[#2a2a2a]' : 'bg-white border-gray-100'}`}>
              ...
              <Link to="/mock-tests" className="inline-flex items-center gap-2 px-6 py-3 bg-[#FFC107] text-black text-[13.5px] font-bold rounded-lg hover:bg-amber-400 transition-all duration-200">
            ...
            <div className={`rounded-xl border overflow-hidden transition-all duration-400 ${isDark ? 'bg-[#141414] border-[#2a2a2a]' : 'bg-white border-gray-100'}`}>
              ...
              <div className={`px-7 py-4 border-t bg-black/[0.01] dark:bg-white/[0.01] ${isDark ? 'border-[#2a2a2a]' : 'border-gray-100'}`}>
                <Link to="/mock-tests" className={`text-[12.5px] font-bold text-[#FFC107] hover:text-amber-500 transition-colors flex items-center gap-1.5 w-max`}>
          ...
          <div className="flex flex-col xl:flex-row gap-10">
            ...
            <div className="flex-1">
              ...
              <div className={`mt-8 flex flex-col sm:flex-row gap-6 items-center justify-between p-7 ${isDark ? 'bg-[#141414] border-[#2a2a2a]' : 'bg-white border-gray-200'} border rounded-xl`}>
                ...
                <Link to="/current-affairs/daily" className="px-7 py-3 bg-[#FFC107] text-black text-[13px] font-[900] uppercase tracking-wider rounded-lg hover:bg-black hover:text-white transition-all duration-300 flex items-center gap-2.5 shrink-0">
      ...
      <section className={`py-12 border-t ${divider} ${altBg}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-4">
            <Link to="/study-materials" className="inline-flex items-center gap-1.5 px-4 py-2 bg-[#FFC107] text-black text-sm font-bold rounded-md hover:bg-amber-400 transition-colors">
      ...
      <section className={`py-16 border-t ${divider} ${isDark ? 'bg-[#0a0a0a]' : 'bg-[#f8fafc]'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className={`mt-8 flex flex-col sm:flex-row items-center justify-between gap-5 rounded-xl px-8 py-6 ${isDark ? 'bg-[#141414] border border-[#2a2a2a]' : 'bg-white border border-gray-100'}`}>
            ...
            <Link to="/register" className="px-7 py-3 bg-[#FFC107] text-black text-[13px] font-[900] uppercase tracking-wider rounded-lg hover:bg-black hover:text-white transition-all duration-300 flex items-center gap-2.5 shrink-0">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`rounded-md border p-8 md:p-12 text-center ${card}`}>
            ...
            <div className="flex flex-wrap justify-center gap-3">
              <Link to="/register" className="inline-flex items-center gap-2 px-6 py-2.5 bg-[#FFC107] text-black text-sm font-bold rounded-md hover:bg-amber-400 transition-colors">
                Register Free
              </Link>
              <Link to="/exams" className={`inline-flex items-center gap-2 px-6 py-2.5 text-sm font-semibold rounded-md border transition-colors ${isDark ? 'border-[#333] text-gray-200 hover:border-[#555]' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
                Browse Exams
              </Link>
              <Link to="/mock-tests" className={`inline-flex items-center gap-2 px-6 py-2.5 text-sm font-semibold rounded-md border transition-colors ${isDark ? 'border-[#333] text-gray-200 hover:border-[#555]' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
                Take Mock Test
              </Link>
            </div>
          </div>
