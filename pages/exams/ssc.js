...
import { sscData } from '../../data/exams/govt-exams-data';
...
const SSCPage = () => {
  ...
  return (
    <div className={`min-h-screen ${cl.bg} ${cl.text} pt-[72px]`} style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <Head>
        <title>SSC Exams 2026 — CGL, CHSL, MTS, GD | Complete Preparation Guide</title>
        <meta name="description" content="Prepare for SSC CGL, CHSL, MTS, GD Constable exams. Eligibility, syllabus, previous year papers, salary and exam strategy." />
      </Head>

      {/* Back Navigation */}
      <div className={`border-b ${cl.border}`}>
        <div className="max-w-[1280px] mx-auto px-5 py-3">
          <Link href="/exams" className={`inline-flex items-center gap-2 text-[12px] font-bold ${cl.sub} hover:text-[${accent}] transition-colors`}>
      ...
      <section className={`border-b ${cl.border}`}>
        <div className="max-w-[1280px] mx-auto px-5 py-10">
          <div className="flex items-start gap-5">
            <I8 src={i8.ssc} alt="SSC" size={48} />
            <div>
              <span className="text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded" style={{ color: accent, backgroundColor: isDark ? 'rgba(21,101,192,0.1)' : '#E3F2FD' }}>Staff Selection Commission</span>
              <h1 className="text-2xl md:text-3xl font-extrabold mt-2">{sscData.shortTitle}</h1>
              <p className={`text-[13px] ${cl.sub} mt-2 max-w-xl leading-relaxed`}>{sscData.tagline}</p>
              <a href={sscData.officialSite} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 mt-3 text-[11px] font-bold" style={{ color: accent }}>
      ...
      <main className="max-w-[1280px] mx-auto px-5 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Vacancies', value: sscData.stats.vacancies, icon: i8.target },
            { label: 'Exams Per Year', value: sscData.stats.examsPerYear, icon: i8.calendar },
            { label: 'Age Limit', value: sscData.stats.ageLimit, icon: i8.doc },
            { label: 'Qualification', value: sscData.stats.qualification, icon: i8.book },
          ].map((s, i) => (
        ...
        <div className={`${cl.surface} border ${cl.border} rounded-lg overflow-hidden`}>
          <div className={`p-5 border-b ${cl.border}`}><h2 className="text-[15px] font-bold flex items-center gap-2"><I8 src={i8.doc} alt="" size={18} /> SSC Examinations</h2></div>
          <div className="divide-y" style={{ borderColor: isDark ? '#30363d' : '#e2e8f0' }}>
            {sscData.exams.map((exam, i) => (
              <div key={i} className="p-5">
                <h3 className="text-[14px] font-bold mb-3">{exam.name}</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {[
                    { label: 'Eligibility', value: exam.eligibility },
                    { label: 'Age Limit', value: exam.ageLimit },
                ...
                <div className="mt-3"><span className={`text-[9px] font-bold uppercase tracking-widest ${cl.sub}`}>Stages</span>
                  <div className="flex flex-wrap gap-2 mt-1.5">
                    {(typeof exam.stages === 'string' ? exam.stages.split(' → ') : exam.stages || []).map((s, k) => <span key={k} className="text-[10px] font-bold px-2.5 py-1 rounded border" style={{ color: accent, borderColor: `${accent}30`, backgroundColor: isDark ? 'rgba(21,101,192,0.06)' : '#E3F2FD' }}>{s}</span>)}
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className={`${cl.surface} border ${cl.border} rounded-lg p-5`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {sscData.importantBooks.map((book, i) => (
              <div key={i} className={`flex items-center gap-2 py-1.5 text-[12px] ${cl.sub}`}><I8 src={i8.check} alt="" size={14} /> {book}</div>
            ))}
          </div>
        </div>
      </main>
