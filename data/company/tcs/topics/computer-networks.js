...
export const theory = [
  {
    ...
    content: "The Open Systems Interconnection (OSI) model is a conceptual framework created by the International Organization for Standardization (ISO) which enables diverse communication systems to communicate using standard protocols. It provides a standard for different computer systems to be able to communicate with each other. It divides the communication process into seven distinct, abstract layers.\n\n| Layer | Name | Function |\n| :--- | :--- | :--- |\n| 7 | **Application** | Directly interacts with software applications. Protocols: HTTP, FTP, SMTP, DNS. |\n| 6 | **Presentation** | Formats, encrypts, and compresses data. Translates app data to network format. |\n| 5 | **Session** | Establishes, maintains, and terminates connections between applications. |\n| 4 | **Transport** | End-to-end communication, error recovery, flow control. Protocols: TCP, UDP. |\n| 3 | **Network** | Routing data packets, logical addressing (IP addresses). Protocols: IP, ICMP, IPsec. |\n| 2 | **Data Link** | Node-to-node data transfer, error detection, physical addressing (MAC). Switches operate here. |\n| 1 | **Physical** | Transmission of raw bit stream over a physical medium (cables, hubs, voltage). |"
  },
  {
    id: "tcp-vs-udp",
    title: "TCP vs UDP: In-depth Comparison",
    content: "Transmission Control Protocol (TCP) and User Datagram Protocol (UDP) are the two most fundamental transport-layer protocols for internet communication. They serve completely different use-cases based on their architecture.\n\n### TCP (Transmission Control Protocol):\nTCP is a connection-oriented protocol. A connection must be established and maintained until the application programs at each end have finished exchanging messages. It determines how to break application data into packets, sends them, and accepts packets from the network layer.\n- **Reliable**: Guarantees delivery by using an acknowledgment system.\n- **Ordered**: Packets arrive in sequence. If they are out of order, TCP reorders them.\n- **Overhead**: Slower due to the 3-way handshake (SYN, SYN-ACK, ACK) and error-checking mechanisms.\n\n### UDP (User Datagram Protocol):\nUDP is connectionless. It sends messages, called datagrams, without establishing a persistent connection and evaluating whether the receiving connection is ready to accept them.\n- **Unreliable**: Fire and forget. No acknowledgments, meaning packets can be lost.\n- **Fast**: Minimal header (8 bytes vs TCP's 20 bytes), no connection setup latency.\n- **Best For**: Live streaming (Twitch, YouTube Live), VoIP calling, and fast-paced multiplayer gaming where a lost packet is better than a delayed packet."
  },
  {
    id: "ip-addressing",
